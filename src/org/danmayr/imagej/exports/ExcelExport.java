...
public class ExcelExport {
    ...
    public static void Export(String outputFolder, FolderResults results) {
        ...
        SXSSFSheet overview = (SXSSFSheet) workBook.createSheet("overview");
        ...
        for (Map.Entry<String, Folder> entry : results.getFolders().entrySet()) {
            ...
            for (Map.Entry<String, Image> entry1 : folder.getImages().entrySet()) {
                ...
                String[] channelNames = image.getChannelNames();
                String[] channelTitles = image.getChannelTitles();
                ...
                for (Map.Entry<Integer, Channel> entry2 : image.getChannels().entrySet()) {
                    ...
                    int row = 2;
                    // Write channel name
                    Row chanelNameRow = imageSheet.createRow(0);
    ...
    public static void Export(String outputFolder, FolderResults results) {
        ...
        for (Map.Entry<String, Folder> folderMap : results.getFolders().entrySet()) {
            ...
            for (Map.Entry<String, Image> imageMap : folder.getImages().entrySet()) {
                ...
                //
                overViewRow = WriteOverviewImageSummery(overviewSheet,sheetName,createHelper, image, folderName, imageName, overViewRow);
    ...
    private static int WriteOverviewHeaderForFolder(SXSSFSheet overviewSheet, Folder folder, int row) {
        ...
        TreeMap<Integer, Pair<String,String[]>> titles = folder.getTitle();
    ...
    /// Write image summary
    ///
    private static int WriteOverviewImageSummery(SXSSFSheet overviewSheet, String sheetName, CreationHelper createHelper, Image image, String folderName, String imageName,
            int row) {
    ...
    private static void WriteImageSheet(SXSSFSheet imgSheet, Image image) {
        ...
        for (Map.Entry<Integer, Channel> entry2 : image.getChannels().entrySet()) {
            ...
            for (Map.Entry<Integer, ParticleInfo> entry3 : channel.getRois().entrySet()) {
                ...
                Row currentRow = imgSheet.getRow(row);
                if (null == currentRow) {
                    currentRow = imgSheet.createRow(row);
                }
