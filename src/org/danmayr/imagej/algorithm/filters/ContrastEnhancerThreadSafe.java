...
public class ContrastEnhancerThreadSafe implements Measurements {
	...
	public void stretchHistogramNormalize(ImagePlus imp, double saturated) {
		this.normalize = true;
		stretchHistogram(imp, saturated);
	}
	...
	public void normalizeMedian(ImageProcessor ip, double medianNew, double medianOld) throws Exception{
		int min2 = 0;
		int max2 = 255;
		int range = 256;
		if (ip instanceof ShortProcessor) {
		...
		int nr = ip.getWidth()*ip.getHeight();
		double factor = (double)medianNew/(double)medianOld;
		IJ.log("Factor " + factor) ;

		for(int i = 0;i<nr;i++){
			int newVal = (int)((double)factor *(double)ip.get(i));
